...
int main() {
    ...
    while (1) {
        ...
        if (a_state != prev_a_state) { // Detecting rising and falling edges of A
            if (a_state == 1) { // Rising edge
                if (b_state == 0) {
                    step_count++;
                    printf("Rising edge: Motor moving Clockwise, Steps: %d\n", step_count);
                } else {
                    step_count--;
                    printf("Rising edge: Motor moving Counterclockwise, Steps: %d\n", step_count);
                }
            } else { // Falling edge
                if (b_state == 1) {
                    step_count++;
                    printf("Falling edge: Motor moving Clockwise, Steps: %d\n", step_count);
                } else {
                    step_count--;
                    printf("Falling edge: Motor moving Counterclockwise, Steps: %d\n", step_count);
                }
            }
        }
